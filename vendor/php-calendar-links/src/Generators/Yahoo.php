...
class Yahoo implements Generator
{
    ...
    public function generate(Link $link)
    {
        ...
        if ($link->allDay) {
	        ...
	        $utcStartDateTime = clone $link->from;
            $utcStartDateTime = $utcStartDateTime->setTimezone(new DateTimeZone('UTC'));

	        $utcEndDateTime = (clone $link->to);
            $utcEndDateTime = $utcEndDateTime->setTimezone(new DateTimeZone('UTC'));
